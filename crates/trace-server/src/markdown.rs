use std::{
    collections::HashMap,
    io::{Cursor, Read, Write},
    path::{Component, Path, PathBuf},
};

use anyhow::{bail, Context, Result};
use chrono::Utc;
use rusqlite::{params, Connection};
use serde::Serialize;
use serde_json::{json, Value};
use uuid::Uuid;
use zip::{write::SimpleFileOptions, ZipArchive, ZipWriter};

const MAX_MARKDOWN_IMPORT_FILES: usize = 10_000;
const MAX_MARKDOWN_IMPORT_UNCOMPRESSED_BYTES: u64 = 512 * 1024 * 1024;

#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct ImportSummary {
    pub workspace_id: String,
    pub workspace_title: String,
    pub imported_notes: u32,
    pub created_relations: u32,
}

#[derive(Debug, Clone)]
pub struct MarkdownExport {
    pub filename: String,
    pub bytes: Vec<u8>,
    pub content_type: &'static str,
}

#[derive(Debug, Clone)]
struct NoteRecord {
    id: String,
    title: String,
    content: String,
    tags: Vec<String>,
    updated_at: String,
    parent_id: Option<String>,
}

#[derive(Debug, Clone)]
struct MarkdownEntry {
    relative_path: PathBuf,
    body: String,
}

#[derive(Debug, Clone)]
struct ParsedMarkdownFile {
    title: Option<String>,
    tags: Vec<String>,
    related: Vec<String>,
    body: String,
}

#[derive(Debug, Clone)]
struct PendingRelation {
    source_id: String,
    target_ref: String,
}

pub fn export_note_markdown(connection: &Connection, note_id: &str) -> Result<MarkdownExport> {
    let record = load_note_by_id(connection, note_id.trim())?;
    let related_ids = load_note_relation_targets(connection, &record.id)?;
    let markdown = compose_markdown(&record, &related_ids)?;

    Ok(MarkdownExport {
        filename: format!("{}.md", sanitize_file_name(&record.title)),
        bytes: markdown.into_bytes(),
        content_type: "text/markdown; charset=utf-8",
    })
}

pub fn export_vault_markdown_zip(connection: &Connection) -> Result<MarkdownExport> {
    let notes = load_all_notes(connection)?;
    let parent_map = load_parent_map(connection)?;
    let relations_map = load_relations_by_source(connection)?;

    let mut writer = ZipWriter::new(Cursor::new(Vec::new()));
    let options = SimpleFileOptions::default().compression_method(zip::CompressionMethod::Deflated);
    let mut seen_paths: HashMap<String, usize> = HashMap::new();

    for note in &notes {
        let related_ids = relations_map.get(&note.id).cloned().unwrap_or_default();
        let markdown = compose_markdown(note, &related_ids)?;
        let path = export_zip_path(note, &parent_map, &mut seen_paths);

        writer
            .start_file(path, options)
            .context("No se pudo crear archivo Markdown dentro del ZIP")?;
        writer
            .write_all(markdown.as_bytes())
            .context("No se pudo escribir Markdown dentro del ZIP")?;
    }

    let cursor = writer
        .finish()
        .context("No se pudo cerrar ZIP de exportacion Markdown")?;
    Ok(MarkdownExport {
        filename: format!("trace-markdown-{}.zip", Utc::now().format("%Y%m%d%H%M%S")),
        bytes: cursor.into_inner(),
        content_type: "application/zip",
    })
}

pub fn import_markdown_zip(connection: &mut Connection, body: &[u8]) -> Result<ImportSummary> {
    if body.is_empty() {
        bail!("El ZIP de Markdown esta vacio");
    }

    let entries = read_markdown_entries(body)?;
    if entries.is_empty() {
        bail!("El ZIP no contiene archivos .md");
    }

    let workspace_title = format!(
        "Importacion Markdown {}",
        Utc::now().format("%Y-%m-%d %H:%M")
    );
    let workspace_id = create_node_id("workspace");
    let workspace_position = next_position_for_parent(connection, None)?;
    let now = now_iso();

    let tx = connection.transaction()?;
    tx.execute(
        "INSERT INTO nodes (id, title, type, parent_id, content, icon, tags, inbox, position, updated_at)
         VALUES (?1, ?2, 'workspace', NULL, NULL, 'workspace', '[]', 0, ?3, ?4)",
        params![workspace_id, workspace_title, workspace_position, now],
    )
    .context("No se pudo crear workspace de importacion")?;

    let mut folder_cache: HashMap<PathBuf, String> = HashMap::new();
    folder_cache.insert(PathBuf::new(), workspace_id.clone());
    let mut lookup: HashMap<String, String> = HashMap::new();
    let mut pending_relations: Vec<PendingRelation> = Vec::new();
    let mut imported_notes = 0_u32;

    for entry in entries {
        let parsed = parse_markdown_file(&entry.body);
        let parent_relative = entry
            .relative_path
            .parent()
            .unwrap_or_else(|| Path::new(""));
        let parent_id =
            ensure_folder_hierarchy(&tx, &workspace_id, parent_relative, &mut folder_cache)?;
        let file_stem = entry
            .relative_path
            .file_stem()
            .and_then(|name| name.to_str())
            .unwrap_or("nota")
            .to_string();
        let title = parsed
            .title
            .clone()
            .filter(|value| !value.trim().is_empty())
            .unwrap_or_else(|| prettify_file_stem(&file_stem));
        let note_id = create_node_id("note");
        let position = next_position_for_parent(&tx, Some(parent_id.as_str()))?;
        let tags_json =
            serde_json::to_string(&parsed.tags).context("No se pudieron serializar tags")?;
        let content_json = markdown_to_blocknote_json(&parsed.body)?;
        let updated_at = now_iso();

        tx.execute(
            "INSERT INTO nodes (id, title, type, parent_id, content, icon, tags, inbox, position, updated_at)
             VALUES (?1, ?2, 'note', ?3, ?4, 'note', ?5, 0, ?6, ?7)",
            params![note_id, title, parent_id, content_json, tags_json, position, updated_at],
        )
        .context("No se pudo insertar nota importada")?;

        lookup.insert(normalize_lookup_key(&note_id), note_id.clone());
        lookup.insert(normalize_lookup_key(&title), note_id.clone());
        lookup.insert(normalize_lookup_key(&file_stem), note_id.clone());
        lookup.insert(
            normalize_lookup_key(
                entry
                    .relative_path
                    .to_string_lossy()
                    .replace('\\', "/")
                    .trim_end_matches(".md")
                    .trim_end_matches(".markdown"),
            ),
            note_id.clone(),
        );

        for relation in parsed.related {
            pending_relations.push(PendingRelation {
                source_id: note_id.clone(),
                target_ref: relation,
            });
        }
        for target in collect_wiki_link_targets(&parsed.body) {
            pending_relations.push(PendingRelation {
                source_id: note_id.clone(),
                target_ref: target,
            });
        }

        imported_notes = imported_notes.saturating_add(1);
    }

    let mut created_relations = 0_u32;
    for relation in pending_relations {
        let key = normalize_lookup_key(&relation.target_ref);
        let Some(target_id) = lookup.get(&key) else {
            continue;
        };
        if target_id == &relation.source_id {
            continue;
        }
        let changed = tx
            .execute(
                "INSERT OR IGNORE INTO note_relations (source_id, target_id) VALUES (?1, ?2)",
                params![relation.source_id, target_id],
            )
            .context("No se pudo crear relacion importada")?;
        if changed > 0 {
            created_relations = created_relations.saturating_add(1);
        }
    }

    tx.commit().context("No se pudo confirmar importacion")?;

    Ok(ImportSummary {
        workspace_id,
        workspace_title,
        imported_notes,
        created_relations,
    })
}

fn read_markdown_entries(body: &[u8]) -> Result<Vec<MarkdownEntry>> {
    let mut archive =
        ZipArchive::new(Cursor::new(body)).context("El ZIP de Markdown no es valido")?;
    if archive.len() > MAX_MARKDOWN_IMPORT_FILES {
        bail!(
            "El ZIP de Markdown excede el limite de {} archivos",
            MAX_MARKDOWN_IMPORT_FILES
        );
    }

    let mut total_uncompressed = 0_u64;
    let mut entries = Vec::new();

    for index in 0..archive.len() {
        let mut file = archive
            .by_index(index)
            .context("No se pudo inspeccionar archivo Markdown")?;
        total_uncompressed = total_uncompressed
            .checked_add(file.size())
            .context("El tamano descomprimido del ZIP no es valido")?;
        if total_uncompressed > MAX_MARKDOWN_IMPORT_UNCOMPRESSED_BYTES {
            bail!("El ZIP de Markdown excede el limite de 512 MiB descomprimido");
        }
        if file.is_dir() {
            continue;
        }

        let Some(relative_path) = safe_zip_path(file.name()) else {
            continue;
        };
        if !is_markdown_path(&relative_path) {
            continue;
        }

        let mut body = String::new();
        file.read_to_string(&mut body)
            .context("No se pudo leer archivo Markdown como UTF-8")?;
        entries.push(MarkdownEntry {
            relative_path,
            body,
        });
    }

    entries.sort_by(|left, right| left.relative_path.cmp(&right.relative_path));
    Ok(entries)
}

fn safe_zip_path(raw: &str) -> Option<PathBuf> {
    let path = Path::new(raw);
    if path.is_absolute() {
        return None;
    }

    let mut safe = PathBuf::new();
    for component in path.components() {
        match component {
            Component::Normal(segment) => safe.push(segment),
            Component::CurDir => {}
            _ => return None,
        }
    }

    (!safe.as_os_str().is_empty()).then_some(safe)
}

fn is_markdown_path(path: &Path) -> bool {
    path.extension()
        .and_then(|extension| extension.to_str())
        .map(|extension| {
            extension.eq_ignore_ascii_case("md") || extension.eq_ignore_ascii_case("markdown")
        })
        .unwrap_or(false)
}

fn load_note_by_id(connection: &Connection, note_id: &str) -> Result<NoteRecord> {
    connection
    .query_row(
      "SELECT id, title, content, tags, updated_at, parent_id FROM nodes WHERE id = ?1 AND type = 'note'",
      params![note_id],
      |row| {
        Ok(NoteRecord {
          id: row.get(0)?,
          title: row.get(1)?,
          content: row.get::<_, Option<String>>(2)?.unwrap_or_default(),
          tags: parse_tags(row.get::<_, Option<String>>(3)?.unwrap_or_else(|| "[]".to_string())),
          updated_at: row.get(4)?,
          parent_id: row.get::<_, Option<String>>(5)?,
        })
      },
    )
    .context("No se encontro la nota solicitada")
}

fn load_all_notes(connection: &Connection) -> Result<Vec<NoteRecord>> {
    let mut statement = connection
        .prepare(
            "SELECT id, title, content, tags, updated_at, parent_id FROM nodes WHERE type = 'note'",
        )
        .context("No se pudo preparar consulta de notas")?;

    let rows = statement
        .query_map([], |row| {
            Ok(NoteRecord {
                id: row.get(0)?,
                title: row.get(1)?,
                content: row.get::<_, Option<String>>(2)?.unwrap_or_default(),
                tags: parse_tags(
                    row.get::<_, Option<String>>(3)?
                        .unwrap_or_else(|| "[]".to_string()),
                ),
                updated_at: row.get(4)?,
                parent_id: row.get::<_, Option<String>>(5)?,
            })
        })
        .context("No se pudo ejecutar consulta de notas")?;

    rows.collect::<Result<Vec<_>, _>>()
        .context("No se pudieron leer notas")
}

fn load_parent_map(connection: &Connection) -> Result<HashMap<String, (String, Option<String>)>> {
    let mut statement = connection
        .prepare("SELECT id, title, parent_id FROM nodes")
        .context("No se pudo preparar consulta de padres")?;
    let rows = statement
        .query_map([], |row| {
            Ok((
                row.get::<_, String>(0)?,
                (row.get::<_, String>(1)?, row.get::<_, Option<String>>(2)?),
            ))
        })
        .context("No se pudo ejecutar consulta de padres")?;

    rows.collect::<Result<HashMap<_, _>, _>>()
        .context("No se pudo construir mapa de padres")
}

fn load_note_relation_targets(connection: &Connection, note_id: &str) -> Result<Vec<String>> {
    let mut statement = connection
        .prepare("SELECT target_id FROM note_relations WHERE source_id = ?1 ORDER BY target_id")
        .context("No se pudo preparar consulta de relaciones")?;
    let rows = statement
        .query_map(params![note_id], |row| row.get::<_, String>(0))
        .context("No se pudo ejecutar consulta de relaciones")?;

    rows.collect::<Result<Vec<_>, _>>()
        .context("No se pudieron leer relaciones")
}

fn load_relations_by_source(connection: &Connection) -> Result<HashMap<String, Vec<String>>> {
    let mut statement = connection
        .prepare("SELECT source_id, target_id FROM note_relations ORDER BY source_id, target_id")
        .context("No se pudo preparar consulta global de relaciones")?;
    let rows = statement
        .query_map([], |row| {
            Ok((row.get::<_, String>(0)?, row.get::<_, String>(1)?))
        })
        .context("No se pudo ejecutar consulta global de relaciones")?;

    let mut map: HashMap<String, Vec<String>> = HashMap::new();
    for row in rows {
        let (source, target) = row.context("No se pudo leer fila de relacion")?;
        map.entry(source).or_default().push(target);
    }
    Ok(map)
}

fn compose_markdown(record: &NoteRecord, related_ids: &[String]) -> Result<String> {
    let mut output = String::new();
    output.push_str("---\n");
    output.push_str(&format!("id: {}\n", record.id));
    output.push_str(&format!(
        "title: {}\n",
        escape_frontmatter_scalar(&record.title)
    ));
    output.push_str("tags:\n");
    if record.tags.is_empty() {
        output.push_str("  -\n");
    } else {
        for tag in &record.tags {
            output.push_str(&format!("  - {}\n", escape_frontmatter_scalar(tag)));
        }
    }
    output.push_str("related:\n");
    if related_ids.is_empty() {
        output.push_str("  -\n");
    } else {
        for related in related_ids {
            output.push_str(&format!("  - {}\n", escape_frontmatter_scalar(related)));
        }
    }
    output.push_str(&format!("updated_at: {}\n", record.updated_at));
    output.push_str("---\n\n");

    output.push_str(&blocks_json_to_markdown(&record.content)?);
    if !output.ends_with('\n') {
        output.push('\n');
    }
    Ok(output)
}

fn blocks_json_to_markdown(content: &str) -> Result<String> {
    if content.trim().is_empty() {
        return Ok(String::new());
    }
    let value: Value =
        serde_json::from_str(content).context("Contenido de nota no es JSON valido")?;
    let blocks = value.as_array().cloned().unwrap_or_default();
    let mut output = String::new();
    render_blocks(&blocks, 0, &mut output);
    Ok(output.trim().to_string())
}

fn render_blocks(blocks: &[Value], indent: usize, output: &mut String) {
    for block in blocks {
        let block_type = block
            .get("type")
            .and_then(Value::as_str)
            .unwrap_or("paragraph");
        let text = extract_text_from_content(block.get("content"));
        let padding = "  ".repeat(indent);

        match block_type {
            "heading" => {
                let level = block
                    .get("props")
                    .and_then(|props| props.get("level"))
                    .and_then(Value::as_u64)
                    .unwrap_or(1)
                    .clamp(1, 6);
                output.push_str(&format!(
                    "{}{} {}\n\n",
                    padding,
                    "#".repeat(level as usize),
                    text
                ));
            }
            "bulletListItem" => {
                output.push_str(&format!("{padding}- {text}\n"));
            }
            "numberedListItem" => {
                output.push_str(&format!("{padding}1. {text}\n"));
            }
            "checkListItem" => {
                let checked = block
                    .get("props")
                    .and_then(|props| props.get("checked"))
                    .and_then(Value::as_bool)
                    .unwrap_or(false);
                output.push_str(&format!(
                    "{padding}- [{}] {}\n",
                    if checked { "x" } else { " " },
                    text
                ));
            }
            "quote" => {
                output.push_str(&format!("{padding}> {text}\n\n"));
            }
            "codeBlock" => {
                let language = block
                    .get("props")
                    .and_then(|props| props.get("language"))
                    .and_then(Value::as_str)
                    .unwrap_or("");
                output.push_str(&format!("{padding}```{language}\n{text}\n{padding}```\n\n"));
            }
            "divider" => {
                output.push_str(&format!("{padding}---\n\n"));
            }
            _ => {
                output.push_str(&format!("{padding}{text}\n\n"));
            }
        }

        if let Some(children) = block.get("children").and_then(Value::as_array) {
            render_blocks(children, indent + 1, output);
            if !output.ends_with('\n') {
                output.push('\n');
            }
        }
    }
}

fn extract_text_from_content(content: Option<&Value>) -> String {
    match content {
        None => String::new(),
        Some(Value::String(value)) => value.trim().to_string(),
        Some(Value::Array(items)) => items
            .iter()
            .map(|item| match item {
                Value::String(value) => value.clone(),
                Value::Object(map) => map
                    .get("text")
                    .and_then(Value::as_str)
                    .unwrap_or_default()
                    .to_string(),
                _ => String::new(),
            })
            .collect::<Vec<_>>()
            .join(" ")
            .split_whitespace()
            .collect::<Vec<_>>()
            .join(" "),
        _ => String::new(),
    }
}

fn parse_markdown_file(raw: &str) -> ParsedMarkdownFile {
    if !raw.starts_with("---") {
        return ParsedMarkdownFile {
            title: None,
            tags: Vec::new(),
            related: Vec::new(),
            body: raw.to_string(),
        };
    }

    let mut lines = raw.lines();
    let first = lines.next().unwrap_or_default();
    if first.trim() != "---" {
        return ParsedMarkdownFile {
            title: None,
            tags: Vec::new(),
            related: Vec::new(),
            body: raw.to_string(),
        };
    }

    let mut frontmatter_lines: Vec<String> = Vec::new();
    let mut body_lines: Vec<String> = Vec::new();
    let mut in_frontmatter = true;

    for line in lines {
        if in_frontmatter && line.trim() == "---" {
            in_frontmatter = false;
            continue;
        }
        if in_frontmatter {
            frontmatter_lines.push(line.to_string());
        } else {
            body_lines.push(line.to_string());
        }
    }

    if in_frontmatter {
        return ParsedMarkdownFile {
            title: None,
            tags: Vec::new(),
            related: Vec::new(),
            body: raw.to_string(),
        };
    }

    let mut title: Option<String> = None;
    let mut tags: Vec<String> = Vec::new();
    let mut related: Vec<String> = Vec::new();
    let mut current_list: Option<&str> = None;

    for line in &frontmatter_lines {
        let trimmed = line.trim();
        if trimmed.is_empty() {
            continue;
        }

        if let Some(rest) = trimmed.strip_prefix("title:") {
            title = Some(rest.trim().trim_matches('"').trim_matches('\'').to_string());
            current_list = None;
            continue;
        }
        if trimmed == "tags:" {
            current_list = Some("tags");
            continue;
        }
        if trimmed == "related:" {
            current_list = Some("related");
            continue;
        }
        if let Some(item) = trimmed.strip_prefix('-') {
            let value = item.trim().trim_matches('"').trim_matches('\'').to_string();
            if value.is_empty() {
                continue;
            }
            match current_list {
                Some("tags") => tags.push(value),
                Some("related") => related.push(value),
                _ => {}
            }
            continue;
        }
        current_list = None;
    }

    ParsedMarkdownFile {
        title,
        tags,
        related,
        body: body_lines.join("\n"),
    }
}

fn markdown_to_blocknote_json(markdown: &str) -> Result<String> {
    let mut blocks: Vec<Value> = Vec::new();
    let lines: Vec<&str> = markdown.lines().collect();
    let mut index = 0usize;

    while index < lines.len() {
        let line = lines[index].trim_end();
        let trimmed = line.trim();
        if trimmed.is_empty() {
            index += 1;
            continue;
        }

        if trimmed.starts_with("```") {
            let language = trimmed.trim_start_matches("```").trim().to_string();
            index += 1;
            let mut code_lines: Vec<String> = Vec::new();
            while index < lines.len() {
                let current = lines[index].trim_end();
                if current.trim().starts_with("```") {
                    index += 1;
                    break;
                }
                code_lines.push(current.to_string());
                index += 1;
            }
            blocks.push(json!({
              "type": "codeBlock",
              "props": { "language": language },
              "content": code_lines.join("\n")
            }));
            continue;
        }

        if trimmed.starts_with('#') {
            let level = trimmed
                .chars()
                .take_while(|char| *char == '#')
                .count()
                .clamp(1, 6);
            let text = trimmed[level..].trim();
            blocks.push(json!({
              "type": "heading",
              "props": { "level": level },
              "content": text
            }));
            index += 1;
            continue;
        }

        if trimmed == "---" || trimmed == "***" {
            blocks.push(json!({ "type": "divider" }));
            index += 1;
            continue;
        }

        if let Some(text) = trimmed.strip_prefix("- [ ] ") {
            blocks.push(json!({
              "type": "checkListItem",
              "props": { "checked": false },
              "content": text.trim()
            }));
            index += 1;
            continue;
        }

        if let Some(text) = trimmed
            .strip_prefix("- [x] ")
            .or_else(|| trimmed.strip_prefix("- [X] "))
        {
            blocks.push(json!({
              "type": "checkListItem",
              "props": { "checked": true },
              "content": text.trim()
            }));
            index += 1;
            continue;
        }

        if let Some(text) = trimmed
            .strip_prefix("- ")
            .or_else(|| trimmed.strip_prefix("* "))
        {
            blocks.push(json!({
              "type": "bulletListItem",
              "content": text.trim()
            }));
            index += 1;
            continue;
        }

        if let Some((_, item_text)) = split_numbered_list_item(trimmed) {
            blocks.push(json!({
              "type": "numberedListItem",
              "content": item_text
            }));
            index += 1;
            continue;
        }

        if let Some(text) = trimmed.strip_prefix("> ") {
            blocks.push(json!({
              "type": "quote",
              "content": text.trim()
            }));
            index += 1;
            continue;
        }

        let mut paragraph_lines = vec![trimmed.to_string()];
        index += 1;
        while index < lines.len() {
            let lookahead = lines[index].trim();
            if lookahead.is_empty() || looks_like_new_block(lookahead) {
                break;
            }
            paragraph_lines.push(lookahead.to_string());
            index += 1;
        }
        blocks.push(json!({
          "type": "paragraph",
          "content": paragraph_lines.join(" ")
        }));
    }

    serde_json::to_string(&blocks).context("No se pudo serializar bloques BlockNote")
}

fn split_numbered_list_item(value: &str) -> Option<(usize, String)> {
    let mut digits = String::new();
    for char in value.chars() {
        if char.is_ascii_digit() {
            digits.push(char);
            continue;
        }
        break;
    }
    if digits.is_empty() {
        return None;
    }
    let rest = value[digits.len()..].trim_start();
    if !rest.starts_with('.') {
        return None;
    }
    let item = rest[1..].trim_start();
    if item.is_empty() {
        return None;
    }
    Some((digits.parse::<usize>().ok()?, item.to_string()))
}

fn looks_like_new_block(value: &str) -> bool {
    value.starts_with('#')
        || value.starts_with("- ")
        || value.starts_with("* ")
        || value.starts_with("- [ ] ")
        || value.starts_with("- [x] ")
        || value.starts_with("- [X] ")
        || value.starts_with("> ")
        || value.starts_with("```")
        || value == "---"
        || value == "***"
        || split_numbered_list_item(value).is_some()
}

fn parse_tags(raw: String) -> Vec<String> {
    match serde_json::from_str::<Value>(&raw) {
        Ok(Value::Array(items)) => items
            .iter()
            .filter_map(Value::as_str)
            .map(str::trim)
            .filter(|tag| !tag.is_empty())
            .map(str::to_string)
            .collect(),
        _ => Vec::new(),
    }
}

fn ensure_folder_hierarchy(
    connection: &Connection,
    workspace_id: &str,
    relative_parent: &Path,
    folder_cache: &mut HashMap<PathBuf, String>,
) -> Result<String> {
    if relative_parent.as_os_str().is_empty() {
        return Ok(workspace_id.to_string());
    }

    let mut current_parent_id = workspace_id.to_string();
    let mut partial = PathBuf::new();

    for segment in relative_parent.components() {
        let Component::Normal(segment_name) = segment else {
            continue;
        };
        partial.push(segment_name);
        if let Some(existing) = folder_cache.get(&partial) {
            current_parent_id = existing.clone();
            continue;
        }

        let folder_name = segment_name.to_string_lossy().to_string();
        let folder_id = create_node_id("folder");
        let position = next_position_for_parent(connection, Some(&current_parent_id))?;
        let now = now_iso();

        connection
            .execute(
                "INSERT INTO nodes (id, title, type, parent_id, content, icon, tags, inbox, position, updated_at)
                 VALUES (?1, ?2, 'folder', ?3, NULL, 'folder', '[]', 0, ?4, ?5)",
                params![folder_id, folder_name, current_parent_id, position, now],
            )
            .context("No se pudo crear carpeta importada")?;

        folder_cache.insert(partial.clone(), folder_id.clone());
        current_parent_id = folder_id;
    }

    Ok(current_parent_id)
}

fn next_position_for_parent(connection: &Connection, parent_id: Option<&str>) -> Result<i64> {
    let max_position = if let Some(parent) = parent_id {
        connection
            .query_row(
                "SELECT MAX(position) FROM nodes WHERE parent_id = ?1",
                params![parent],
                |row| row.get::<_, Option<i64>>(0),
            )
            .context("No se pudo leer posicion maxima")?
    } else {
        connection
            .query_row(
                "SELECT MAX(position) FROM nodes WHERE parent_id IS NULL",
                [],
                |row| row.get::<_, Option<i64>>(0),
            )
            .context("No se pudo leer posicion maxima en raiz")?
    };

    Ok(max_position.unwrap_or(-1) + 1)
}

fn resolve_parent_segments(
    note: &NoteRecord,
    parent_map: &HashMap<String, (String, Option<String>)>,
) -> Vec<String> {
    let mut segments: Vec<String> = Vec::new();
    let mut cursor = note.parent_id.clone();
    let mut safe_guard = 0;
    while let Some(parent_id) = cursor {
        if safe_guard > 256 {
            break;
        }
        safe_guard += 1;
        let Some((title, parent_parent)) = parent_map.get(&parent_id) else {
            break;
        };
        segments.push(title.clone());
        cursor = parent_parent.clone();
    }
    segments.reverse();
    segments
}

fn export_zip_path(
    note: &NoteRecord,
    parent_map: &HashMap<String, (String, Option<String>)>,
    seen_paths: &mut HashMap<String, usize>,
) -> String {
    let mut segments = resolve_parent_segments(note, parent_map)
        .into_iter()
        .map(|segment| sanitize_file_name(&segment))
        .filter(|segment| !segment.is_empty())
        .collect::<Vec<_>>();
    segments.push(format!("{}.md", sanitize_file_name(&note.title)));
    let path = segments.join("/");
    unique_zip_path(path, seen_paths)
}

fn unique_zip_path(path: String, seen_paths: &mut HashMap<String, usize>) -> String {
    let count = seen_paths.entry(path.clone()).or_insert(0);
    *count += 1;
    if *count == 1 {
        return path;
    }

    let Some((stem, extension)) = path.rsplit_once('.') else {
        return format!("{path} ({count})");
    };
    format!("{stem} ({count}).{extension}")
}

fn sanitize_file_name(value: &str) -> String {
    let trimmed = value.trim();
    let fallback = if trimmed.is_empty() {
        "untitled"
    } else {
        trimmed
    };
    let mut out = String::new();
    for char in fallback.chars() {
        let invalid = matches!(char, '<' | '>' | ':' | '"' | '/' | '\\' | '|' | '?' | '*');
        if invalid || char.is_control() {
            out.push('_');
        } else {
            out.push(char);
        }
    }
    let cleaned = out.trim_matches('.').trim();
    if cleaned.is_empty() {
        "untitled".to_string()
    } else {
        cleaned.to_string()
    }
}

fn escape_frontmatter_scalar(value: &str) -> String {
    if value.contains(':') || value.contains('#') || value.contains('"') || value.contains('\'') {
        format!("\"{}\"", value.replace('"', "\\\""))
    } else {
        value.to_string()
    }
}

fn normalize_lookup_key(value: &str) -> String {
    let normalized: String = value
        .to_lowercase()
        .chars()
        .map(|char| {
            if char.is_ascii_alphanumeric() {
                char
            } else {
                ' '
            }
        })
        .collect();
    normalized.split_whitespace().collect::<Vec<_>>().join(" ")
}

fn collect_wiki_link_targets(body: &str) -> Vec<String> {
    let mut targets = Vec::new();
    let mut rest = body;
    while let Some(start) = rest.find("[[") {
        let after_start = &rest[start + 2..];
        let Some(end) = after_start.find("]]") else {
            break;
        };
        let target = after_start[..end].trim();
        if !target.is_empty() {
            targets.push(target.to_string());
        }
        rest = &after_start[end + 2..];
    }
    targets
}

fn prettify_file_stem(value: &str) -> String {
    let value = value.replace(['_', '-'], " ");
    let prettified = value.split_whitespace().collect::<Vec<_>>().join(" ");
    if prettified.is_empty() {
        "Nota".to_string()
    } else {
        prettified
    }
}

fn create_node_id(prefix: &str) -> String {
    format!("{prefix}-{}", Uuid::new_v4())
}

fn now_iso() -> String {
    Utc::now().to_rfc3339()
}

#[cfg(test)]
mod tests {
    use super::*;
    use trace_core::schema::ensure_trace_schema;

    #[test]
    fn markdown_roundtrip_imports_zip_and_exports_vault() {
        let mut connection = Connection::open_in_memory().expect("sqlite opens");
        ensure_trace_schema(&connection).expect("schema exists");

        let source_zip = markdown_zip(vec![
            (
                "projects/alpha.md",
                "---\ntitle: Alpha\ntags:\n  - rust\nrelated:\n  - Beta\n---\n\n# Alpha\n\nLinks [[Beta]]",
            ),
            ("projects/beta.md", "# Beta\n\n- item"),
        ]);

        let summary = import_markdown_zip(&mut connection, &source_zip).expect("markdown imports");
        assert_eq!(summary.imported_notes, 2);
        assert!(summary.created_relations >= 1);

        let export = export_vault_markdown_zip(&connection).expect("vault exports");
        assert_eq!(export.content_type, "application/zip");
        assert!(export.filename.ends_with(".zip"));
        assert!(!export.bytes.is_empty());
    }

    #[test]
    fn safe_zip_path_rejects_traversal() {
        assert!(safe_zip_path("../secrets.md").is_none());
        assert!(safe_zip_path("notes/ok.md").is_some());
    }

    fn markdown_zip(entries: Vec<(&str, &str)>) -> Vec<u8> {
        let mut writer = ZipWriter::new(Cursor::new(Vec::new()));
        let options =
            SimpleFileOptions::default().compression_method(zip::CompressionMethod::Stored);
        for (path, body) in entries {
            writer.start_file(path, options).expect("file starts");
            writer.write_all(body.as_bytes()).expect("file writes");
        }
        writer.finish().expect("zip finishes").into_inner()
    }
}
