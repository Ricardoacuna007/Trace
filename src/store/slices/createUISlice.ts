...
async function persistUIConfig(
  ...
  set({
    ...
    traceCodeRunnerSettings: nextConfig.code_runner,
...
export const createUISlice = (deps: UISliceDeps): SliceCreator<UISlice> => (set, get) => ({
  ...
  traceCodeRunnerSettings: parseTraceConfig(deps.defaultTraceConfigJson).code_runner,
  ...
  hydrateUIFromConfig: (configJson) => {
    ...
    set((state) => ({
      ...
      traceCodeRunnerSettings: parsed.code_runner,
  ...
  updateTraceCodeRunnerSettings: async (patch) => {
    await persistUIConfig(set, get, deps, (config) => ({
      ...config,
      code_runner: {
        ...config.code_runner,
